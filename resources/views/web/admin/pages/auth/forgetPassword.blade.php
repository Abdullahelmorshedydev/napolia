...
<head>
    ...
    <!-- Title -->
    <title> {{ __('admin/auth/forget.title') }} </title>
                                ...
                                <div class="col-md-10 col-lg-10 col-xl-9 mx-auto">
                                    <div class="card-sigin">
                                        ...
                                        <div class="main-signup-header">
                                            <h2 class="text-primary">{{ __('admin/auth/forget.get_started') }}</h2>
                                            <h5 class="font-weight-normal mb-4">
                                                {{ __('admin/auth/forget.waiting') }}
                                            </h5>
                                            <form action="{{ route('admin.auth.forget.send') }}" method="POST">
                                                @csrf
                                                <div class="form-group">
                                                    <label>{{ __('admin/auth/forget.email') }}</label>
                                                    <input class="form-control" name="email"
                                                        placeholder="{{ __('admin/auth/forget.email_place') }}"
                                                        type="text">
                                                        @error('email')
                                                ...
                                                <button class="btn btn-main-primary btn-block">
                                                    {{ __('admin/auth/forget.send') }}
                                                </button>
                                            </form>
                                            <div class="main-signup-footer mt-5">
                                                <p>
                                                    {{ __('admin/auth/forget.forget_it') }}
                                                    <a href="{{ route('admin.auth.login.show') }}">
                                                        {{ __('admin/auth/forget.send_me_back') }}
                                                    </a>
                                                    {{ __('admin/auth/forget.sign_in') }}
                                                </p>
                                            </div>
                                        </div>
                                    </div>
